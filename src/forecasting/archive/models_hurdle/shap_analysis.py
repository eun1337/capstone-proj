...
# 강수량/공휴일 피처는 rename 전/후 데이터 모두에서 동작하도록 후보 이름을 순서대로 시도한다.
# 공휴일_D0/D-1/D+1 + B센터_명절휴무(거래-조인 기반, B는 항상 0인 구조적 결함 있었음)는
# 공휴일_W0/W-1/W+1(순수 캘린더 기준 A/B 공통 단일 변수)로 완전히 대체됨.
WEATHER_COL_CANDIDATES = {
    ...
    "공휴일_W0": ["공휴일_W0", "공휴일_D0"],
    "공휴일_W-1": ["공휴일_W-1", "공휴일_D-1"],
    "공휴일_W+1": ["공휴일_W+1", "공휴일_D+1"],
    "B센터_명절휴무(구)": ["B센터_명절휴무"],
