...
sys.path.insert(0, str(BASE_DIR / "src" / "forecasting" / "archive" / "feature_engineering" / "economic_indicators"))
...
def forecast_origin_check() -> dict:
    split_path = BASE_DIR / "src" / "forecasting" / "archive" / "feature_engineering" / "split_train_val_test.py"
...
def covid_check(df: pd.DataFrame) -> list[dict]:
    ...
    feature_src = read_source(
        BASE_DIR / "src" / "forecasting" / "archive" / "feature_engineering" / "feature_external_interaction_concat.py"
    )
    split_src = read_source(
        BASE_DIR / "src" / "forecasting" / "archive" / "feature_engineering" / "split_train_val_test.py"
    )
