...
lookback과 나머지 모델·학습 파라미터는 고정한다.
...
from src.forecasting.deep_learning.tft import trainer as tft_trainer
...
if LOOKBACK != 13:
    raise RuntimeError(f"tft.config.LOOKBACK={LOOKBACK}가 frozen spec 13과 다름")
if tuple(sorted(HIDDEN_SIZE_CHOICES)) != (8, 16):
    raise RuntimeError(f"tft.config.HIDDEN_SIZE_CHOICES={HIDDEN_SIZE_CHOICES}가 frozen spec (8,16)과 다름")
if MAX_EPOCHS != 2:
    raise RuntimeError(f"tft.config.MAX_EPOCHS={MAX_EPOCHS}가 frozen spec 2와 다름")
if LSTM_LAYERS != 1:
    raise RuntimeError(f"tft.config.LSTM_LAYERS={LSTM_LAYERS}가 frozen spec 1과 다름")
_GRID_SIZE = len(HIDDEN_SIZE_CHOICES)
if _GRID_SIZE != 2:
    raise RuntimeError(f"TFT grid size={_GRID_SIZE}가 frozen spec 2와 다름")


def suggest_tft_hp(trial: optuna.Trial) -> dict:
    return {
        "hidden_size": trial.suggest_categorical("hidden_size", list(HIDDEN_SIZE_CHOICES)),
        "lookback": LOOKBACK, "hidden_continuous_size": HIDDEN_CONTINUOUS_SIZE,
        "attention_head_size": ATTENTION_HEAD_SIZE, "dropout": DROPOUT,
        "learning_rate": LEARNING_RATE, "gradient_clip_val": GRADIENT_CLIP_VAL,
...
def _run_folds(hp: dict, sub_a: pd.DataFrame, folds: list, horizon: int, seed: int,
    ...
    """하나의 TFT configuration을 4-fold에서 학습·평가하고 native OOF를 반환한다."""
    oof_frames, fold_metrics = [], []
    for fold in folds:
        fold_id = fold["fold"]
        checkpoint_dir = checkpoint_root / f"{trial_label}_fold{fold_id}"
        try:
            result = tft_trainer.train_and_evaluate_fold(
                sub_a, fold, horizon, hp["lookback"],
                hidden_size=hp["hidden_size"], hidden_continuous_size=hp["hidden_continuous_size"],
                attention_head_size=hp["attention_head_size"], dropout=hp["dropout"],
                learning_rate=hp["learning_rate"], gradient_clip_val=hp["gradient_clip_val"],
...
def main() -> None:
    parser = argparse.ArgumentParser(description="TFT P13 Final HPO (2023, A center)")
    parser.add_argument("--output-dir", default="outputs/hpo/tft")
