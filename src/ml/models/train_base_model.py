...
def main():
    ...
    print(f"[Base Model 1단계-분류] 양성비율={y_cls_train.mean():.1%} scale_pos_weight={scale_pos_weight:.3f}")
    ...
    cls_model, best_cls_lr, best_cls_leaves, cls_grid_table = grid_search_classifier(
        X_cls_train, y_cls_train, X_cls_val, y_cls_val, scale_pos_weight
    )
    print(f"[Base Model 1단계-분류] 채택: lr={best_cls_lr} num_leaves={best_cls_leaves}")

    # ---- 2단계: 조건부회귀기 튜닝 (target_h1>0만) ----
